    ...
    if(isset($_POST['search'])){
      ...
      if($_POST['txt_option'] != ""){
        ...
        $sql = "SELECT * FROM tbl_pos_expense_list AS A
                        ...
                        WHERE A.exp_date_record BETWEEN '$dateStart' AND '$dateEnd' AND A.exp_expense_category='$v_option'";  
        $result = $connect->query($sql);
      }else{
           $sql = "SELECT * FROM tbl_pos_expense_list AS A
                        LEFT JOIN tbl_pos_expense_category AS B ON A.exp_expense_category=B.exca_id
                        LEFT JOIN tbl_pos_employee AS C ON A.exp_employee=C.emp_id
                        WHERE A.exp_date_record BETWEEN '$dateStart' AND '$dateEnd'";  
      $result = $connect->query($sql);
      }            
    }else{
        $v_date = date('Y-m-d');
        $sql = "SELECT * FROM tbl_pos_expense_list AS A
                        LEFT JOIN tbl_pos_expense_category AS B ON A.exp_expense_category=B.exca_id
                        LEFT JOIN tbl_pos_employee AS C ON A.exp_employee=C.emp_id
                        WHERE A.exp_date_record='$v_date'";  
  $result = $connect->query($sql);
    }
...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-image fa-fw"></i> Report Expense by Category</h2>
        </div>
    </div>
    <br>
    <br>
            ...
            <form class="form-inline" method = "post" action="">
                ...
                <div class="form-group">
                    <select class="form-control" name = "txt_option"> 
                      ...
                      <?php
                            $v_select = mysqli_query($connect,"SELECT * FROM tbl_pos_expense_category");
                            while ($row1 = mysqli_fetch_assoc($v_select)) {
                                if($row1['exca_id'] == @$_POST['txt_option'])
