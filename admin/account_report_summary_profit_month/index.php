...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i> <?= $lang_text['reportProfitByMonth'][$lang] ?></h2>
        </div>
    </div>    
    <br>
    <br>
    <div class="row">
        <form action="<?= $_SERVER['PHP_SELF'] ?>" method="get">
            <div class="col-sm-2">
                <div class="input-group date" data-provide="datepicker" data-date-format="yyyy-mm">
                    <input autocomplete="off" name="txt_date_start" value="<?= @$_GET['txt_date_start'] ?>" REQUIRED type="text" class="form-control" placeholder="month from">
            ...
            <div class="col-sm-2">
                <div class="input-group date" data-provide="datepicker" data-date-format="yyyy-mm">
                    <input autocomplete="off" name="txt_date_end" value="<?= @$_GET['txt_date_end'] ?>" REQUIRED type="text" class="form-control" placeholder="month to">
                    <div class="input-group-addon">
                        <span class="glyphicon glyphicon-th"></span>
                    </div>
                </div>
            ...
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline " id="sample_2" role="grid" aria-describedby="sample_2_info">
                ...
                <tbody>
                    <?php
                        ...
                        $sum_expense_khr = 0;
                        while ($date<=$v_date_e){
                            $get_income = $connect->query("SELECT SUM(ai_amount_dollar) as sum_income_usd,SUM(ai_amount_riel) as sum_incime_khr FROM tbl_account_income WHERE DATE_FORMAT(ai_date,'%Y-%m')='$date'");
                            $row_income = mysqli_fetch_object($get_income);
                            $get_expense = $connect->query("SELECT SUM(ae_amount_dollar) as sum_expense_usd,SUM(ae_amount_riel) as sum_expense_khr FROM tbl_account_expense WHERE DATE_FORMAT(ae_date,'%Y-%m')='$date'");
